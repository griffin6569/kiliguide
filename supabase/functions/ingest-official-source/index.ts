...
const allowedHost = (host: string) => host === "dkut.ac.ke" || host.endsWith(".dkut.ac.ke");
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!allowedHost(source.hostname)) return Response.json({ error: "Only official dkut.ac.ke sources may be ingested." }, { status: 400, headers: cors });
    ...
    const { data: document, error: documentError } = await admin.from("documents").insert({ title: documentTitle, category, storage_path: path, file_type: "txt", uploaded_by: user.id, notify_on_ready: Boolean(notifyOnReady), metadata: { source_url: source.href, retrieved_at: new Date().toISOString(), source_type: "official_web" } }).select("id").single();
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Official-source ingestion failed." }, { status: 500, headers: cors });
  }
});
